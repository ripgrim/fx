...
fn runProviderLogin(alloc: Allocator, cfg: Config, provider: model_provider.ProviderId) !void {
    switch (provider) {
        .claude => {
            var status = (try credentials.probeClaudeStatus(alloc)) orelse return error.ClaudeCodeUnavailable;
            defer status.deinit(alloc);
            if (!status.isSubscription()) return error.ClaudeSubscriptionRequired;
        },
    ...
    switch (parsed_command) {
        ...
        .login => |rest| {
            ...
            try writeStdout(deps, switch (login_provider) {
                .claude => "Claude Code subscription selected.\n",
