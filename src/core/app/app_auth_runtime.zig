...
pub fn Runtime(comptime App: type) type {
    return struct {
            ...
            switch (selection) {
                ...
                .missing => try app.writeDomainNotice(.{
                    ...
                    .body = switch (provider) {
                        .claude => "Claude Code is unavailable. Run claude auth login with a subscription.",
            ...
            const requested_provider = if (std.mem.trim(u8, target, " \t\r\n").len == 0)
                ...
                provider_catalog.parse(std.mem.trim(u8, target, " \t\r\n")) orelse {
                    try writeAuthNotice(app, .{
                        .topic = "auth",
                        .tone = .warning,
                        .body = "Usage: /logout [vercel|codex|grok|claude]",
                    });
                    return;
                };
        ...
        fn finishProviderSwitch(app: *App, task: *auth_runtime.ProviderPreparation) !bool {
            ...
            if (task.credential == null and !hostManagesAuth(app)) {
                if (request.allow_login) {
                    switch (target) {
                        .claude => try app.writeDomainNotice(.{ .topic = "auth", .tone = .warning, .body = "Run claude auth login with a subscription, then select Claude again." }, true),
        ...
        fn beginCredentialRepair(
            ...
            if (failure.source == .claude_subscription) {
                try app.writeDomainNotice(.{ .topic = "auth", .tone = .warning, .body = "Run claude auth login with a subscription, then retry." }, true);
                return;
            }
    ...
    for (cases) |case| {
        inline for ([_]model_provider.ProviderId{ .codex, .grok }) |provider| {
            ...
            switch (provider) {
                ...
                // Only the OAuth providers are exercised here; fx runs no
