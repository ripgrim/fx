...
pub const runtime_context =
    ...
    "Capture the live page, inspect the source tree, then prepare_import. Select the underlying Paper tools for scoped access, then call execute with each pending operation hash. fx loads the exact payload, checks permission, executes Paper and records its real result. Inspect after each action for the next operation. Never supply receipts or recreate an existing SVG or image from perception. " ++
