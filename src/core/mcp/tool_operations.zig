...
pub const Operations = struct {
    ...
    fn callTool(
        ...
        const raw_frame_cap = toolResponseFrameCap(snapshot.prefixed_name, max_tool_result_bytes);
    ...
    pub fn mcpResponseFrameCap(max_tool_result_bytes: usize) usize {
        return frameCap(max_tool_result_bytes);
    }

    fn toolResponseFrameCap(name: []const u8, model_bytes: usize) usize {
        const bytes = if (std.mem.eql(u8, name, "mcp_paper_get_screenshot")) @max(model_bytes, 16 * 1024 * 1024) else model_bytes;
        return frameCap(bytes);
    }

    fn frameCap(max_tool_result_bytes: usize) usize {
    ...
    fn callToolLegacyHttp(
        ...
        const raw_frame_cap = toolResponseFrameCap(prefixed_name, max_tool_result_bytes);
