...
import type { ComponentReport } from "./provenance";
...
export class Inspector {
  ...
  async publish(directory: string, snapshot: VerificationSnapshot) {
    ...
    let auto_open = false;
    if ((snapshot.source_image || snapshot.component_report) && snapshot.canvas_image) {
...
[hidden]{display:none!important}.views.components{grid-template-columns:minmax(0,1fr)}.components figure:not(:last-child){display:none}.views.components canvas{width:auto;max-width:100%;max-height:65vh;margin-inline:auto}.components h2 span{display:none}#component-detail{display:flex;gap:24px;align-items:start;flex-wrap:wrap;margin-top:20px}#component-detail img{display:block;max-width:min(100%,480px);height:auto;margin-top:10px;border:1px solid var(--line);border-radius:8px}#coverage{font-size:12px;color:var(--muted)}#findings button[aria-pressed=true]{border-color:var(--accent);background:var(--subtle)}
...
<p id="coverage" hidden aria-live="polite"></p><div id="findings"></div><div id="component-detail" aria-live="polite"></div><pre id="finding-detail" style="white-space:pre-wrap;overflow-wrap:anywhere" aria-live="polite"></pre><details id="diagnostics"><summary>Details</summary><p class="muted">Pixel diff is approximate.</p><div id="token-findings"></div><p id="provenance" class="muted"></p></details></main>
...
function componentMode(active){document.querySelector('.views').classList.toggle('components',active);$('coverage').hidden=!active;$('source-toggle').hidden=active;overlayMode.hidden=active;changedLegend.hidden=active;if(!active)$('component-detail').replaceChildren()}
function drawComponents(){if(!images||!componentOverlay)return;const board=images[1],region=crop||{x:0,y:0,width:board.width,height:board.height};for(const id of ['source','paper','diff']){const c=$(id);c.width=Math.ceil(region.width);c.height=Math.ceil(region.height);const ctx=c.getContext('2d');ctx.drawImage(board,region.x,region.y,c.width,c.height,0,0,c.width,c.height);if(id==='diff'&&$('pixels').checked){ctx.drawImage(componentOverlay,region.x,region.y,c.width,c.height,0,0,c.width,c.height);ctx.save();ctx.translate(-region.x,-region.y);ctx.strokeStyle='rgb(167 119 42)';ctx.setLineDash([4,4]);for(const item of snapshot.component_report.instances)if(item.rect&&item.status==='unavailable')ctx.strokeRect(item.rect.x+.5,item.rect.y+.5,item.rect.width-1,item.rect.height-1);ctx.restore()}}$('expand').disabled=false;$('full').disabled=!crop;document.querySelector('.views').setAttribute('aria-busy','false');if(lightbox.open)syncLightbox()}
async function showComponents(next){componentMode(true);const report=next.component_report;$('coverage').textContent=report.checked+' / '+report.linked+' linked instances checked'+(report.unlinked_regions?' · '+report.unlinked_regions+' unlinked regions':'');$('state').textContent=report.instances.some(item=>item.status==='drift')?'Component drift':report.instances.some(item=>item.status!=='consistent')?'Partially checked':'Consistent';$('findings').replaceChildren();$('token-findings').replaceChildren();$('component-detail').replaceChildren();$('finding-detail').textContent='';for(const item of report.instances){const button=document.createElement('button');button.textContent=item.label+' · '+({consistent:'Consistent',drift:'Drift',unlinked:'Unlinked',unavailable:'Not checked'}[item.status]);button.className=item.status==='drift'?'red':item.status==='consistent'?'green':'';button.setAttribute('aria-pressed','false');button.onclick=()=>{for(const b of $('findings').children)b.setAttribute('aria-pressed',String(b===button));const r=item.rect;crop=r?{x:Math.max(0,r.x-16),y:Math.max(0,r.y-16),width:r.width+32,height:r.height+32}:undefined;$('component-detail').replaceChildren();$('finding-detail').textContent=item.message||'';for(const [label,src] of [['Code reference',item.reference_image],['Paper instance',item.paper_image]])if(src){const figure=document.createElement('figure'),caption=document.createElement('figcaption'),image=document.createElement('img');caption.textContent=label;image.alt=label+': '+item.label;image.src=src;image.style.background=item.backdrop||'transparent';figure.append(caption,image);$('component-detail').append(figure)}draw()};(item.status==='unlinked'?$('token-findings'):$('findings')).append(button)}const [board,overlay]=await Promise.all([load(next.canvas_image),load(report.overlay_image)]);images=[board,board];componentOverlay=overlay;draw()}
function draw(){if(snapshot?.component_report){drawComponents();return}if(!images)return;$('expand').disabled=false;$('full').disabled=!crop;document.querySelector('.views').setAttribute('aria-busy','false');const [a,b]=images;const region=crop||{x:0,y:0,width:Math.max(a.width,b.width),height:Math.max(a.height,b.height)};const width=Math.max(1,Math.ceil(region.width)),height=Math.max(1,Math.ceil(region.height));if(width*height>32000000)throw Error('Image exceeds preview limit');
