...
Verification uses lossless Paper PNG exports and ignores tiny dimension rounding (up to 1/32 CSS px) and low-intensity pixel noise (up to 24/255 per channel), consistently in the gate and overlay. Anti-alias filtering recognizes intermediate edge shades between unchanged neighboring colors. Detail is the default overlay: near-opaque pixel-level highlights preserve text shapes with lightly softened edges. Regions and unsmoothed Pixels remain optional. Amber marks mixed regions, red source residuals, and green Paper residuals. Missing borders remain independently checked; small details are not discarded based on their percentage of the image.
