...
Verification uses lossless Paper PNG exports and ignores tiny dimension rounding (up to 1/32 CSS px) and low-intensity pixel noise (up to 24/255 per channel), consistently in the gate and overlay. The viewer defaults to stable highlighted regions; select Pixels for the exact residuals. Amber marks mixed changes, red source residuals, and green Paper residuals. Missing borders remain independently checked; small details are not discarded based on their percentage of the image.
